...
- set `DEBUG = True` and disable `SECURE_SSL_REDIRECT` in a local override, or
...
2. `SECRET_KEY=… ALLOWED_HOSTS=petar-dev.com,.petar-dev.com python manage.py migrate`
3. `cd django_code && caddy run --config Caddyfile` (auto-issues and renews Let's Encrypt certs)
